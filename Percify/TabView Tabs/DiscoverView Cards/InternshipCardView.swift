...
struct InternshipCardView: View {
        ...
        cardContent
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 32, style: .continuous)
                    .strokeBorder(
                        LinearGradient(
                            colors: [Color.blue.opacity(1.0), Color.blue.opacity(0.4), Color.blue.opacity(0.8)],
                            startPoint: .top,
                            endPoint: .bottom
                        ),
                        lineWidth: 6.5
            ...
            .accessibilityLabel(Text("Internship card for \(recruitment.titleText) at \(recruitment.companyName) in \(recruitment.location)"))
    ...
    private var internBadgePill: some View {
        HStack(spacing: 4) {
            Image(systemName: "studentdesktop")
                .font(.subheadline)
            Text("長期インターン")
        ...
        .padding(.leading, 12)
        .background(
            LinearGradient(colors: [.blue, .blue.opacity(1.0)], startPoint: .top, endPoint: .bottom)
                .clipShape(UnevenRoundedRectangle(topLeadingRadius: 0, bottomLeadingRadius: 0, bottomTrailingRadius: 32, topTrailingRadius: 0))
                .overlay(
    ...
    private var industryAndTypeRow: some View {
        HStack(alignment: .firstTextBaseline) {
            ...
            HStack {
                labelChip(text: "種別", symbol: "square.grid.2x2.fill", chipColor: .blue)
                    .fontWeight(.semibold)
    ...
    private var salaryRow: some View {
        ZStack {
            ...
            if isChartVisible {
                Chart {
                    ForEach(Array(chartData.enumerated()), id: \.offset) { index, value in
                        ...
                        .foregroundStyle(
                            LinearGradient(
                                colors: [Color.blue.opacity(0.15), Color.blue.opacity(0.00)],
                                startPoint: .top,
                                endPoint: .bottom
                            )
                        )
            ...
            // Salary content with internship-style labels
            HStack(alignment: .firstTextBaseline, spacing: 0) {
                VStack(alignment: .leading, spacing: 6) {
                    labelChip(text: "日給", chipColor: .blue)
                        .fontWeight(.semibold)
                ...
                VStack(alignment: .trailing, spacing: 6) {
                    labelChip(text: "内定実績", chipColor: .blue)
                        .fontWeight(.semibold)
        ...
        .onAppear {
            ...
            chartData = (0..<8).map { index in
                let baseIncrease = Double(index) * 7.0
            ...
            Timer.scheduledTimer(withTimeInterval: 3.0, repeats: true) { _ in
                withAnimation(.easeInOut(duration: 2.0)) {
                    chartData = chartData.enumerated().map { index, value in
                        let smallVariation = Double.random(in: -1...2)
    ...
    private var likeOverlay: some View {
        ZStack {
            LinearGradient(
                colors: [Color.blue.opacity(0.2), Color.blue.opacity(0.8)],
    ...
    ZStack {
        ...
        InternshipCardView(
            recruitment: Recruitment(
                companyName: "Google Japan",
                companyLogo: "Google",
                badgeText: "Percify特別選考",
                titleText: "Google STEP Internship - ソフトウェアエンジニアリング",
                industryLeft: "NULL",
                industryRight: "IT・テクノロジー",
                typeLeft: "NULL",
                typeRight: "インターン",
                pay1Label: "Pay1",
                pay1Value: "5,000",
